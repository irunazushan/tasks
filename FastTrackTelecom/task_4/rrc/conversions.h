...
#define ntoh_int32_buf(bUF)                                     \
  ((*((uint8_t*)bUF)) << 24) | ((*((uint8_t*)bUF + 1)) << 16) | \
...
#define ntoh_int24_buf(bUF)                                 \
  ((*(uint8_t*)bUF << 16) | ((*((uint8_t*)bUF + 1)) << 8) | \
   (*((uint8_t*)bUF + 2)))

#define ntoh_int16_buf(bUF) ((*((uint8_t*)bUF) << 8) | (*((uint8_t*)bUF + 1)))
...
#define PLMNID_TO_MCC_MNC(oCTETsTRING, mCC, mNC, mNCdIGITlENGTH)              \
  do {                                                                        \
    ...
    mNC = (mNCdIGITlENGTH == 2 ? 0                                            \
                               : ((oCTETsTRING)->buf[1] >> 4 & 0x0F) * 100) + \
...
#define BIT_STRING_TO_MaskedIMEISV(bITsTRING, mACRO)                         \
  do {                                                                       \
    ...
    DevCheck((bITsTRING)->bits_unused == 0, (bITsTRING)->bits_unused, 0, 0); \
    mACRO = ((bITsTRING)->buf[0] << 56) + ((bITsTRING)->buf[1] << 48) +      \
            ((bITsTRING)->buf[2] << 40) + ((bITsTRING)->buf[3] << 32) +      \
            ((bITsTRING)->buf[4] << 24) + ((bITsTRING)->buf[5] << 16) +      \
            ((bITsTRING)->buf[6] << 8) + ((bITsTRING)->buf[7]);              \
  } while (0)
